...
class PIIDetector:
    ...
    PATTERNS = {
        ...
        "singpass": r"\bSingPass\s*ID\s*[:\-]?\s*[A-Za-z0-9]+\b",
        "cpf": r"\bCPF\s*(?:Account\s*)?(?:No\.?|Number)?\s*[:\-]?\s*\d{3}[- ]?\d{5}[- ]?\d{2}\b",
        "phone_sg": r"\b(?:\+65[-.\s]?)?[689]\d{3}[-.\s]?\d{4}\b",
        "postal_sg": r"\bSingapore\s+\d{6}\b|\b\d{6}\s+Singapore\b",
        "passport_sg": r"\bE\d{7}[A-Z]\b",
    }
    ...
    async def scan(self, content: Any, path: str = "root") -> PIIDetectionResult:
        ...
        Returns:
            PIIDetectionResult with actual PII matches
        """
